...
procdumpP1(struct proc *p, char *state_string)
{
  ...
  elapsedmilliseconds = ticks - p->start_ticks;
  elapsedseconds = elapsedmilliseconds / 1000;
  elapsedmilliseconds = elapsedmilliseconds % 1000;

  char* zero = "";
  if(elapsedmilliseconds < 100 && elapsedmilliseconds >= 10)
    zero = "0";
  if(elapsedmilliseconds < 10)
    zero = "00";

  cprintf("%d\t%s\t%s%d.%s%d\t%s\t%d\t", p->pid, p->name, "     ", elapsedseconds, zero, elapsedmilliseconds, states[p->state], p->sz);
  return;
}
#endif
